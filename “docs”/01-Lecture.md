...
5. Powerful tools for communication your results.
...
Once we create these variables, they will be stored in the global environment and will be available to use for further operations. Now R know that `x = 4` and `y = 10`.
...
As mentioned, R stores files in the working directory. To check where is the working directory on your machine, use `getwd()` function:
